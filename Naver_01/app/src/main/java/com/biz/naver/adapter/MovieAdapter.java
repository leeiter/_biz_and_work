...
public class MovieAdapter extends RecyclerView.Adapter {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        // 이미지 세팅 잠시 보류
        String imgeLink = movieVOList.get(position).getImage();
        if (!imgeLink.isEmpty()) {
            // imageLink에 있는 이미지를 다운로드하여
            // mHolder.m_image ImageView에 표시하라
            Picasso.get().load(imgeLink).into(mHolder.m_image);

            Glide.with(mHolder.itemView.getContext()).load(imgeLink).into(mHolder.m_image);
        }
    }
